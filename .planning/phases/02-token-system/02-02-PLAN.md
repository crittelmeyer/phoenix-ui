...
Purpose: This makes the token system usable -- developers can write `bg-primary text-foreground` in components and get the design system's curated colors. Dark mode "just works" with a class toggle. The @theme directive IS the Tailwind configuration -- no JS preset file needed in Tailwind v4.
...
**CSS variable naming pattern:** Style Dictionary's `css` transformGroup transforms DTCG token paths to CSS variables by joining path segments with hyphens and prefixing with `--`. Specifically:

- `color.neutral.50` becomes `--color-neutral-50`
- `color.semantic.background` becomes `--color-semantic-background`
- `color.semantic.primary-foreground` becomes `--color-semantic-primary-foreground`
- `spacing.1` becomes `--spacing-1`
- `typography.size.base` becomes `--typography-size-base`
- `radii.md` becomes `--radii-md`

Read the actual `packages/tokens/dist/tokens.css` output from Plan 01 to confirm the exact generated variable names, then map them in the @theme block. The examples above show the expected pattern but the real names depend on the token JSON structure from Plan 01.
...
- This serves as visual proof the token system works
  ...
  <verify>
- `pnpm dev` from root starts without errors
- Visit localhost:5173 -- page renders with token-based colors
- Inspect element: CSS variables from tokens.css are applied
- Click theme toggle: colors switch to dark mode values
- Refresh page in dark mode: no flash of light theme (inline script works)
- Open new incognito window: theme defaults to OS preference
- `pnpm turbo run build` succeeds from root
- `pnpm turbo run typecheck` passes
- Verify token sourcing: In browser DevTools, inspect a `bg-primary` element. The computed background-color MUST show an OKLCH value (e.g., `oklch(0.45 0.2 260)`) proving colors come from the token system, NOT Tailwind's default palette. If the computed value is a hex or rgb color not matching the token OKLCH values, the @theme wiring is incorrect.
  </verify>
  <done>
  apps/web consumes design tokens via Tailwind v4 @theme directive. Utility classes like bg-primary and text-foreground render correct token colors sourced from OKLCH token CSS variables. Dark mode toggle switches themes, persists to localStorage, respects OS preference, and loads without flash.
  </done>
  </task>
